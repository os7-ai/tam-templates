...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { claimed } = await claimStage(sb, user.id, engagementId, 'layer1');
    if (!claimed) {
      return { statusCode: 409, body: JSON.stringify({ error: 'Layer 1 قيد التنفيذ بالفعل لهذا الـEngagement' }) };
    }

    await saveJson(sb, user.id, engagementId, 'inputs/layer1_images.json', { images });
    ...
    if (!res.ok && res.status !== 202) {
      await finishStage(sb, user.id, engagementId, 'layer1', { status: 'error', error: `تعذّر بدء المهمة الخلفية (HTTP ${res.status})` });
