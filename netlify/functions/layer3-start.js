...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { claimed } = await claimStage(token, user.id, engagementId, 'layer3');
    if (!claimed) {
    ...
    if (!res.ok && res.status !== 202) {
      await finishStage(token, user.id, engagementId, 'layer3', { status: 'error', error: `تعذّر بدء المهمة الخلفية (HTTP ${res.status})` });
