...
async function getPipelineStatus(token, userId, engagementId) {
  const existing = await loadJson(token, userId, engagementId, 'pipeline_status.json');
...
async function claimStage(token, userId, engagementId, stage) {
  const pipelineStatus = await getPipelineStatus(token, userId, engagementId);
  ...
  await saveJson(token, userId, engagementId, 'pipeline_status.json', pipelineStatus);
  return { claimed: true, pipelineStatus, attemptId };
}

async function finishStage(token, userId, engagementId, stage, patch) {
  const pipelineStatus = await getPipelineStatus(token, userId, engagementId);
  pipelineStatus.stages[stage] = {
    ...pipelineStatus.stages[stage],
    ...patch,
...
async function updateStageProgress(token, userId, engagementId, stage, progress) {
  const pipelineStatus = await getPipelineStatus(token, userId, engagementId);
